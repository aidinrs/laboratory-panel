...
function httpRequest(request) {
  if (request.method === 'POST') {
    ...
    return axios.post(request.url, request.formData, {headers: request.headers ? {...headers, ...request.headers} : headers});
  }
    // console.log({he:request.headers})
  return axios.get(request.url, {headers: request.headers ? request.headers: undefined});
}
