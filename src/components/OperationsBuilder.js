...
let operation = (ops, index, dispatch) => {
  ...
  if (ops.length > 1) {
    removeLink = <p className="TransactionOpMeta__remove">
      <a onClick={() => dispatch(removeOperation(op.id))}>حذف</a>
    </p>;
  }
  ...
  return <div className="TransactionOp" key={op.id}>
    ...
    <div className="TransactionOp__config TransactionOpConfig optionsTable">
      <OptionsTablePair label={<span>نوع عملیات <HelpMark href="https://www.stellar.org/developers/learn/concepts/list-of-operations.html" /></span>}>
